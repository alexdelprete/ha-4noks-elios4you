...
Configure an optional script that automatically executes when the device stops responding. This enables
automated recovery actions like restarting your WiFi router or power-cycling network equipment.
...
**Use Case:** When the Elios4you device becomes unreachable (e.g., due to WiFi issues), the integration
will automatically run your recovery script after the configured number of failures, potentially
restoring connectivity without manual intervention.
