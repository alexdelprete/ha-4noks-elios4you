...
This is a hotfix release that addresses an error discovered in v0.2.0-beta.1 where the integration
would log an error during unload due to a missing `close()` method in the `Elios4YouAPI` class.
...
When unloading the integration (during Home Assistant shutdown, integration removal, or reload),
the code attempted to call `coordinator.api.close()` but the `Elios4YouAPI` class didn't have a
`close()` method. Only the internal `E4Uclient` (telnet client) had this method.
