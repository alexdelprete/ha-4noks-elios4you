...
This beta release migrates the integration from a bundled synchronous `telnetlib` implementation
to the `telnetlib3` async client library. This architectural change eliminates Home Assistant event
loop blocking during telnet I/O operations.
...
1. **`custom_components/4noks_elios4you/api.py`** - Major rewrite
...
1. **`custom_components/4noks_elios4you/__init__.py`**
...
1. **`custom_components/4noks_elios4you/const.py`**
   - Version bump: `0.3.0-beta.2` -> `0.4.0-beta.1`

1. **`custom_components/4noks_elios4you/manifest.json`**
   - Version bump: `0.3.0-beta.2` -> `0.4.0-beta.1`
...
This release was developed with assistance from Claude AI (Anthropic's Opus 4.5 model) via the
Claude Code VSCode extension. The AI performed:
...
All code changes were reviewed and approved by the human developer (@alexdelprete). The AI acted
as a development assistant, not an autonomous developer.
